import { temporizador, contador } from "./temporizador.js";
...
import concursoLng from "./concurso.js";
...
$start.addEventListener("click", () => {
  reloj = setInterval(() => {
    let fecha = new Date();
    $time.innerHTML = `${fecha.getHours() < 10 ? "0" + fecha.getHours() : fecha.getHours()
...
d.addEventListener("DOMContentLoaded", () => {
  ...
  concursoLng("btn_concurso", ".opcion", "resConcurso");
  controlSlider();
...
d.addEventListener("click", (e) => {
 let detencion;
  ...
  if (e.target.id === "envio-fecha-inicial") {
    let fecha = d.getElementById("fecha-inicial").value;
    
    if (fecha) {
     detencion= contador("msg-ini-date", fecha,"div-cont1","div-cont2");
    } else {
      d.getElementById('msg-ini-date').innerHTML = `No has introducido una fecha`;
    }
  }
   if(e.target.id==="detener-fecha-inicial"){
     clearInterval(detencion);
     d.getElementById("div-cont1").style.display="block";
     d.getElementById("div-cont2").style.display="none";
   }
})
